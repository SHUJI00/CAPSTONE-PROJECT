...
$userId = $_SESSION['user']['login_id'];

try {
    // Prepare the SQL query to retrieve the image blob
    $query = "SELECT profile_img FROM staff_acc WHERE login_id = ?";
    $stmt = $pdo->prepare($query);
    $stmt->bindParam(1, $userId, PDO::PARAM_INT);
    $stmt->execute();
    $profileImageBlob = $stmt->fetchColumn();

    // Convert the blob to base64 encoded string if it exists and store it in the session
    $_SESSION['user']['profile_img'] = $profileImageBlob ? base64_encode($profileImageBlob) : '';
                    ...
                    <div>
                        <button type="button" class="flex text-sm bg-gray-800 focus:ring-4 focus:ring-gray-300 w-8 h-8 rounded-full" aria-expanded="false" data-dropdown-toggle="dropdown-user">
                            ...
                            <?php if (!empty($_SESSION['user']['profile_img'])): ?>
                                <img src="data:image/jpeg;base64,<?php echo $_SESSION['user']['profile_img']; ?>" alt="User Image" class="w-8 h-8 rounded-full">
                            <?php else: ?>
                                <img src="path/to/default/image.jpg" alt="Default Image" class="w-8 h-8 rounded-full"> <!-- Provide a path to a default image -->
                            <?php endif; ?>
                        </button>
                    </div>
  ...
  <a href="/susers/build/php/app/staff/index.php" class="flex md:me-24 mb-5 justify-center w-full">
     ...
     <div class="h-full px-3 pb-4 overflow-y-auto bg-green-700">
        <ul class="space-y-2 font-medium">
           <li>
              <a href="/susers/build/php/app/staff/index.php" class="flex items-center p-2 text-gray-900 rounded-lg bg-green-800 group">
...
<p class="mr-2 text-gray-600">Filter:</p>
   ...
   <input datepicker datepicker-buttons datepicker-autoselect-today type="text" name="start" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5" placeholder="Select date start">
   </div>
   ...
   <input datepicker datepicker-buttons datepicker-autoselect-today type="text" name="end" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5" placeholder="Select date end">
   </div>
